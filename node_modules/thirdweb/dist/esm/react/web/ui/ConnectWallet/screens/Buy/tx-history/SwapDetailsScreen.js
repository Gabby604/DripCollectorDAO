    ...
    return (_jsxs("div", { children: [isPartialSuccess && gotToken ? (
            ...
            _jsx(TokenInfoRow, { chainId: toToken.chainId, client: client, label: "Receive", tokenAmount: toToken.amount, tokenSymbol: toToken.symbol || "", tokenAddress: toToken.address })), lineSpacer, _jsx(TokenInfoRow, { chainId: fromToken.chainId, client: client, label: "Pay", tokenAmount: fromToken.amount, tokenSymbol: fromToken.symbol || "", tokenAddress: fromToken.address }), lineSpacer, _jsxs(Container, { flex: "row", center: "y", style: {
                    justifyContent: "space-between",
                            ...
                            justifyContent: "space-between",
                        }, children: [_jsx(Text, { children: "Status" }), _jsx(Container, { flex: "row", gap: "xs", center: "y", children: _jsx(Text, { color: statusMeta.color, children: statusMeta.status }) })] })] })), lineSpacer, fromChainQuery.data?.explorers?.[0]?.url && sourceTxHash && (_jsxs(ButtonLink, { fullWidth: true, variant: "outline", href: `${fromChainQuery.data.explorers[0].url}/tx/${sourceTxHash}`, target: "_blank", gap: "xs", style: {
                    fontSize: fontSize.sm,
                    padding: spacing.sm,
                }, children: ["View on ", fromChainQuery.data.name, " Explorer", _jsx(ExternalLinkIcon, { width: iconSize.sm, height: iconSize.sm })] })), destinationTxHash &&
                sourceTxHash !== destinationTxHash &&
                toChainQuery.data?.explorers?.[0]?.url && (_jsxs(_Fragment, { children: [_jsx(Spacer, { y: "sm" }), _jsxs(ButtonLink, { fullWidth: true, variant: "outline", href: `${toChainQuery.data.explorers[0].url}/tx/${destinationTxHash}`, target: "_blank", gap: "xs", style: {
                            fontSize: fontSize.sm,
                            padding: spacing.sm,
                        }, children: ["View on ", toChainQuery.data.name, " Explorer", _jsx(ExternalLinkIcon, { width: iconSize.sm, height: iconSize.sm })] })] }))] }));
}
